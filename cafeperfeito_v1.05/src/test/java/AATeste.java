...
//                new ProdutoCodigoBarraDAO().getAll(ProdutoCodigoBarra.class, null, null, null, null)
//        );
...
//                        codigoBarraDAO.persiste(produtoCodigoBarra);
//                    } catch (IOException e) {
//                        e.printStackTrace();
...
//                });
        ...
        ObservableList<Colaborador> colaboradorObservableList = FXCollections.observableArrayList(
                colaboradorDAO.getAll(Colaborador.class, null, null, null, null)
        );
                ...
                .forEach(colaborador -> {
                    try {
                        ...
                        fileTmp[0] = ServiceFileFinder.finder("/Users/thiagomacedo/Desktop/MySqlTestes/",
                                colaborador.apelidoProperty().getValue(), "\\.(jpg|jpeg|png)");
                        if (fileTmp[0] != null)
                            colaborador.setImgColaborador(ServiceImage.getBobFromImage(fileTmp[0], 250, 250));
                        if (colaborador.getImgColaborador() != null)
                            colaboradorDAO.persiste(colaborador);
        ...
        ObservableList<Produto> produtoObservableList = FXCollections.observableArrayList(
                produtoDAO.getAll(Produto.class, null, null, null, null)
        );
        cont[0] = 0;
                    ...
                    try {
                        ...
                        if (produto.getProdutoCodigoBarraList().size() > 0) {
                            for (ProdutoCodigoBarra cod : produto.getProdutoCodigoBarraList()) {
                                ...
                                if (fileTmp[0] != null)
                                    produto.setImgProduto(ServiceImage.getBobFromImage(fileTmp[0], 250, 250));
                            }
...
//                });
    ...
    private static Blob imagem2Blob(Image image) throws IOException, SQLException {
        return new SerialBlob(ServiceImage.getInputStreamFromImage(image).readAllBytes());
    }
}